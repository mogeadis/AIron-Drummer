...
for track in tracks:
    ...
    track_features = get_features(audio,measures,configuration,features_cols)
    ...
    drum_track = separate_tracks(midi)[0]
    drum_track = drum_mapping(drum_track,drum_map)
    drum_onsets = get_drum_onsets(drum_track,drum_parts)
    track_targets = get_targets(drum_onsets,measures,configuration,targets_cols)
