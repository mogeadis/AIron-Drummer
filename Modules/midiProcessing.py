...
from audioProcessing import get_audio_segments
from featureExtraction import get_bar_features
from neuralNetwork import build_encoder_decoder,inference
import pandas as pd
from mido import MidiFile,MidiTrack,Message,MetaMessage

# =======================================================================================================
# Function: get_tempo_events(...)
            ...
            if track[msg].is_meta == True:
                if track[msg].type == 'set_tempo':
                    ...
                    if len(tempo_events_sec) != 0:
                        total_seconds += ticks2seconds(tpb,bpm,last_ticks)
...
# =======================================================================================================
# Function: get_time_signature_events(...)
# Description: This function returns all time signature change events that occur in a MIDI file
# =======================================================================================================
def get_time_signature_events(midi):

    # Get Tempo Events
    tempo_events = get_tempo_events(midi)
            ...
            if track[msg].is_meta == True:
                if track[msg].type == 'time_signature':
                    ...
                    if len(time_signature_events_sec) != 0:
                        ...
                        total_seconds = tempo_events_sec[last_tempo_event_id] + ticks2seconds(tpb,bpm,last_ticks)
...
def get_events(midi):
    ...
    time_signature_events = get_time_signature_events(midi)
        ...
        for msg in range(num_of_msgs):
            ...
            if track[msg].is_meta == True:
                if track[msg].type == 'end_of_track':
                    if total_ticks >= end_of_file_ticks:
                        end_of_file_ticks = total_ticks

    return end_of_file_ticks
...
def get_measures(midi):
    ...
    end_of_file_ticks = get_end_of_file_ticks(midi)
    ...
    while end_ticks < end_of_file_ticks:
        ...
        start_sec = end_sec
        end_sec = start_sec + ticks2seconds(tpb,bpm,(end_ticks - start_ticks))
...
def separate_tracks(midi):
    ...
    drum_track_id = get_drum_track_id(midi)
...
# =======================================================================================================
# Function: get_drum_segments(...)
# Description: This function splits drum onsets into segments
# =======================================================================================================
def get_drum_segments(drum_onsets,measures):
...
# =======================================================================================================
# Function: quantize_drum_onsets(...)
# Description: This function quantizes drum onsets to note positions
# =======================================================================================================
def quantize_drum_onsets(drum_onsets,note_positions):
...
# =======================================================================================================
# Function: get_bar_targets(...)
# Description: This function creates target variables for a single bar of a drum track
# =======================================================================================================
def get_bar_targets(drum_onsets,start_ticks,end_ticks,num,den,configuration,return_ticks = False):
    ...
    note_positions = get_note_positions(start_ticks,end_ticks,num_of_notes,rounding = True)[0]

    # Quantize Drum Onsets
    bar_targets = quantize_drum_onsets(drum_onsets,note_positions)
...
# =======================================================================================================
def get_targets(drum_onsets,measures,configuration,targets_cols):
    ...
    drum_segments = get_drum_segments(drum_onsets,measures)
    ...
    for bar in range(num_of_measures):
        ...
        # Bar Targets
        bar_targets,ticks = get_bar_targets(drum_segment,start_ticks,end_ticks,num,den,configuration,return_ticks = True)
...
# =======================================================================================================
# Function: create_drum_track(...)
# Description: This function creates a MIDI drum track
# =======================================================================================================
def create_drum_track(drum_onsets,drum_parts,end_of_file_ticks):
...
# =======================================================================================================
# Function: generate_drum_track(...)
# Description: This function generates a novel MIDI drum track
# =======================================================================================================
def generate_drum_track(midi,audio,model,drum_parts,configuration,bars = (1,None)):

    # Build Encoder & Decoder
    encoder,decoder = build_encoder_decoder(model)
    ...
    for bar in range(num_of_measures):
        ...
        # Bar Features
        bar_features = get_bar_features(audio_segment,start_sec,end_sec,num,den,configuration)
    ...
    # Create Drum Onsets DataFrame
    drum_onsets = pd.DataFrame(predictions,columns = drum_part_names,index = get_ticks_index(measures,note_value,triplets))

    # Filter Drum Onsets
    drum_onsets = filter_onsets(drum_onsets)

    # Create Drum Track
    end_of_file_ticks =  measures_end_ticks[-1] - measures_start_ticks[0]
    drum_track = create_drum_track(drum_onsets,drum_parts,end_of_file_ticks)

    return drum_track

# =======================================================================================================
# Function: create_events_track(...)
# Description: This function creates a MIDI track with all tempo and time signature change messages
# =======================================================================================================
def create_events_track(midi):
...
# =======================================================================================================
def save_midi(midi_path,midi_type,midi_tpb,midi_tracks):
...
# =======================================================================================================
# Function: save_drum_track(...)
# Description: This function saves a drum track into a MIDI file
# =======================================================================================================
def save_drum_track(drum_track,midi,midi_path):

    # Create Events Track
    events_track = create_events_track(midi)
    ...
    # Save MIDI File
    save_midi(midi_path,midi_type,midi_tpb,midi_tracks)